...
function mostrarVegano() {
  const veganoEntradas = entradas.filter((entrada) =>
    entrada.item.includes("Apto")
  );
  ...
  const veganoPromociones = promociones.filter((promocion) =>
    promocion.item.includes("Apto")
  );
  ...
  for (const veganoEntrada of veganoEntradas) {
    console.log(
      "------ENTRADAS------" +
        veganoEntrada.item +
        " - $ " +
        veganoEntrada.precio
    );
  }

  for (const veganoWok of veganoWoks) {
    console.log(
      "------WOKS------" + veganoWok.item + " - $ " + veganoWok.precio
    );
  }

  for (const veganoSushi of veganoSushis) {
    console.log(
      "------SUSHIS------" + veganoSushi.item + " - $ " + veganoSushi.precio
    );
  }
  ...
  for (const veganoPromocion of veganoPromociones) {
    console.log(
      "------PROMOCIONES------" +
        veganoPromocion.item +
        " - $ " +
        veganoPromocion.precio
    );
  }
  console.log("------------------------------------");
...
function armarPedido() {
  ...
  let opcion = Number(
    prompt("Ingrese el numero de la opcion \n" + fraseDeMenu + "5 - Salir")
  );
  if (opcion == 5) {
    salir();
  ...
  function compararPalabras(textoIngresado, textoEsperado) {
    let esIgual =
      textoIngresado != null && textoIngresado.toLowerCase() === textoEsperado;
    return esIgual;
  }
